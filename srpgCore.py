...
class srpgCore( object ):
	...
	race_folder = None
	language_keygroup = { }
	tf_class_names = {1: "Scout", 2: "Sniper", 3: "Soldier", 4: "Demoman", 5: "Medic", 6: "Heavy", 7: "Pyro", 8: "Spy", 9: "Engineer"}
	...
	def __init__( self ):
		...
		dbConfig = ConfigObj( es.getAddonPath( 'srpg' ) + '/config/database.ini' )
		...
		for steam_id in self.players:
			...
			if player.has_key( 'info' ):
				...
				for class_id in player:
					...
					if class_id != 'info':
						query = "INSERT INTO `srpg`.`client` VALUES( NULL, '%s', '%s', %s, %s ) \
							ON DUPLICATE KEY UPDATE `level` = %s, `experience` = %s ;" % \
						        ( client_steam_id, class_id, data[ 'level' ], data[ 'experience' ], data[ 'level' ], data[ 'experience' ] )
						self.db._execute( query )
			else:
	...
	def getPlayerClass( self, userid ):
		...
		for ac_player_manager in ac_entitylist:
			...
			ac_class = es.getindexprop( ac_player_manager, "CTFPlayerResource.m_iPlayerClass.%03d" % int( ac_player.get( 'index' ) ) )
